...
powsub$Voltage<-as.numeric(powsub$Voltage)
powsub$Global_reactive_power<-as.numeric(powsub$Global_reactive_power)


par(mfrow = c(2,2), mar = c(4, 4, 2, 1), oma = c(0, 0, 2, 0))
with(powsub, {
  plot(ts, gap, pch="", ylab="Global Active Power")
  lines(ts, gap)
  plot(ts, Voltage, pch="", xlab="datetime")
  lines(ts, Voltage)
  plot(ts, gap, pch="", ylab="Energy sub metering", xlab="", ylim=c(0,38))
  ...
  plot(ts, Global_reactive_power, pch="")
  lines(ts, Global_reactive_power)
})
